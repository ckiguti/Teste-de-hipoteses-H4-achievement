#' ---
#' title: "ANCOVA test for `pos.score`~`pre.score`+`scenario`*`achievement`"
...
#' 
## ---- echo=FALSE-------------------------------------------------------------------------------------------------------------------------------
car::Boxplot(`pos.score` ~ `scenario`*`achievement`, data = dat[["pos.score"]], id = list(n = Inf))
...
#' ### Textual Report
#' 
#' After controlling the linearity of covariance "pre.score", ANCOVA tests with independent between-subjects variables "scenario" (gamified, non.gamified) and "achievement" (lower, upper) were performed to determine statistically significant difference on the dependent varibles "pos.score". For the dependent variable "pos.score", there was statistically significant effects in the factor "pre.score" with F(1,25)=45.361, p<0.001 and ges=0.645 (effect size) and in the factor "scenario" with F(1,25)=70.381, p<0.001 and ges=0.738 (effect size) and in the factor "achievement" with F(1,25)=8.845, p=0.006 and ges=0.261 (effect size).
#' 
#' 
#' 
#' Pairwise comparisons using the Estimated Marginal Means (EMMs) were computed to find statistically significant diferences among the groups defined by the independent variables, and with the p-values ajusted by the method "bonferroni". For the dependent variable "pos.score", the mean in the scenario="gamified" (adj M=8.502 and SD=1.061) was significantly different than the mean in the scenario="non.gamified" (adj M=6.808 and SD=0.9) with p-adj<0.001; the mean in the scenario="gamified" (adj M=9.219 and SD=0.707) was significantly different than the mean in the scenario="non.gamified" (adj M=7.368 and SD=1.069) with p-adj<0.001; the mean in the achievement="lower" (adj M=8.502 and SD=1.061) was significantly different than the mean in the achievement="upper" (adj M=9.219 and SD=0.707) with p-adj=0.019.
#' 
#' 
#' 
#' ## Tips and References
